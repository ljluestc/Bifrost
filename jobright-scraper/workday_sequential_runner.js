...
const STRICT_PASSIVE_MODE = true; // PASSIVE MODE (Clicks Disabled for Training)
const LEARN_ONLY_MODE = true; // LEARNING MODE (Recordings Enabled)
    ...
    const reloadJobs = () => {
        ...
        for (const job of validJobs) {
            ...
            if (uLower.includes('smartrecruiters')) {
                ...
                tier2.push(job);
            } else if (uLower.includes('greenhouse')) {
                tier3.push(job);
            } else {
                tier4.push(job);
            }
        ...
        console.log(`\n📊 Queue Priorities (Updated):`);
        console.log(`   🔴 Tier 0 (RETRY FAILED): ${tier0.length} jobs`);
        console.log(`   🟡 Tier 1 (SmartRecruiters): ${tier1.length} jobs`);
        console.log(`   🔵 Tier 2 (Workday):      ${tier2.length} jobs`);
        console.log(`   🟢 Tier 3 (Greenhouse):   ${tier3.length} jobs [PRIORITY]`);
        console.log(`   ⚪ Tier 4 (Others):       ${tier4.length} jobs [PAUSED]`);

        // USER REQUEST: [Greenhouse (Fresh)] -> [Failed] -> [SR] -> [Workday]
        const activeQueue = [...tier3, ...tier0, ...tier1, ...tier2];
        console.log(`   ----------------------------------------`);
        console.log(`   Σ  Total Active Queue: ${activeQueue.length}`);
