...
(function () {
    ...
    function findAndClickButton() {
        try {
            ...
            const searchContexts = [
                { doc: document, iframe: null },
                ...Array.from(iframes).map(iframe => {
                    try {
                        return { doc: iframe.contentDocument || iframe.contentWindow.document, iframe };
                    } catch (e) { return null; }
                }).filter(Boolean)
            ];

            searchContexts.forEach(({ doc, iframe }) => {
                // Broaden selection to include divs/spans that might be buttons
                const elements = doc.querySelectorAll('button, div, span, [role="button"]');
                ...
                elements.forEach(element => {
                    ...
                    // Detect Accept/Proceed or Run buttons
                    const isAccept = lowerText.includes('accept') || lowerText.includes('proceed');
                    const isRun = lowerText === 'run' || (lowerText.length < 10 && lowerText.includes('run'));

                    const matchesClass = className.includes('hover:bg-ide-button-hover') ||
                        className.includes('bg-ide-button-bac') ||
                        className.includes('ide-run') ||
                        className.includes('button') || // Broad class match
                        lowerText === 'run';

                    // Only click if it's visible and doesn't look like a false positive
                    if ((isAccept || isRun) && matchesClass &&
                        element.offsetWidth > 0 && element.offsetHeight > 0) {
                        foundButtons.push({ button: element, iframe });
                    }
                });
            });
    ...
    function clickButtons(buttons, scrollIntoView) {
        buttons.forEach(({ button }, index) => {
            console.log(`  🖱️  Aggressively clicking element ${index + 1} (${button.tagName})`);
            ...
            const countElem = document.getElementById('click-count');
            if (countElem) countElem.textContent = state.totalClicks;
        });
    }
