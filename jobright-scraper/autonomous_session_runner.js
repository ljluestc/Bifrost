...
async function fillGreenhouseForm(page) {
    ...
    try {
        ...
        // MFA / VERIFICATION CODE AUTOMATION
        const codeInput = page.locator('input[id*="code"], input[name*="code"], input[aria-label*="code"]');
        if (await codeInput.count() > 0 && await codeInput.first().isVisible()) {
            console.log("   🔐 MFA Field Detected! Attempting Auto-Gmail Retrieval...");
            // Check if we already have it filled?
            const val = await codeInput.first().inputValue();
            if (!val) {
                const code = await getGreenhouseCodeFromGmail(page.context()); // Pass Context/Browser
                if (code) {
                    await codeInput.first().fill(code, t);
                    console.log(`   ✅ Auto-Filled Code: ${code}`);
                }
            }
        }
...
async function getGreenhouseCodeFromGmail(browser) {
    console.log("   📧 Checking Gmail for Greenhouse Code...");
    let page;
    try {
        page = await browser.newPage();
        await page.goto("https://mail.google.com/mail/u/0/#search/Greenhouse", { waitUntil: 'networkidle', timeout: 15000 });

        // Wait for list to load
        try {
            await page.waitForSelector('tr.zA', { timeout: 5000 });
        } catch (e) {
            console.log("   (Gmail list not found)");
            await page.close();
            return null;
        }

        // Get the most recent email row
        const rows = await page.locator('tr.zA').all();
        if (rows.length === 0) { console.log("   (No emails found)"); await page.close(); return null; }

        // Click the first one
        await rows[0].click();
        await page.waitForTimeout(2000); // Wait for open

        // Scrape body for code
        const body = await page.innerText('body');

        // Match 6 digit codes
        // Often "Your verification code is: 123456"
        const matches = body.match(/\b\d{6}\b/g);
        let code = null;
        if (matches && matches.length > 0) {
            // Heuristic: Last match is often the footer zip code, first match might be valid. 
            // Greenhouse codes are usually prominent.
            // Let's look for digits surrounded by "verification code" context if possible, or just take the first distinct 6-digit number that isn't a year/zip.
            // Simple approach: Take the first one that appears in the email content area.
            code = matches[0];
            console.log(`   ✅ Found potential code: ${code}`);
        } else {
            console.log("   (No code pattern found in email)");
        }
...
(async () => {
    ...
    try {
        ...
        await page.exposeFunction('triggerGetCode', async () => {
            console.log("   ⚡ GET CODE TRIGGERED");
            const code = await getGreenhouseCodeFromGmail(browser);
            return code;
        });
    ...
    const INJECT_RECORDING_SCRIPT = async (targetPage = page) => {
    ...
    const INJECT_CONTROLS_SCRIPT = async (targetPage = page) => {
        await targetPage.evaluate(() => {
            ...
            title.innerText = '🟢 LEARNING (AUTO-PLAY)';
            ...
            const codeBtn = createBtn('📧 GET CODE', '#3498db', async () => {
                console.log("GET CODE CLICKED");
                codeBtn.innerText = 'Checking...';
                if (window.triggerGetCode) {
                    const code = await window.triggerGetCode();
                    if (code) {
                        codeBtn.innerText = `Code: ${code}`;
                        alert(`Greenhouse Code: ${code}\n(Copied to clipboard logic not available here, please type it)`);
                        // Try to auto-fill if input is present
                        const input = document.querySelector('input[id*="code"], input[name*="code"]');
                        if (input) {
                            input.value = code;
                            input.dispatchEvent(new Event('input', { bubbles: true }));
                            input.dispatchEvent(new Event('change', { bubbles: true }));
                            codeBtn.innerText = 'Filled!';
                        }
                    } else {
                        codeBtn.innerText = 'Not Found';
                        setTimeout(() => codeBtn.innerText = '📧 GET CODE', 3000);
                    }
                }
            });
    ...
    const reloadJobs = () => {
        ...
        // 4. TIERS: PRIORITIZE BY COMPANY FREQUENCY (High to Low)
        // 4. TIERS: PRIORITIZE BY COMPANY FREQUENCY
        const companyCounts = {};
        validJobs.forEach(j => {
        ...
        const sortedCompanies = Object.entries(companyCounts).sort((a, b) => b[1] - a[1]).slice(0, 15);
        sortedCompanies.forEach(([comp, count], idx) => {
        ...
        console.log(`   ----------------------------------------`);
        console.log(`   Σ  Total Active Learning Batch: ${activeQueue.length}`);

        console.log("\n   📋 CURRENT BATCH MANIFEST (Top 10):");
        activeQueue.slice(0, 10).forEach((j, idx) => {
            const c = j.company || 'Unknown';
            console.log(`   ${idx + 1}. [${j.title.substring(0, 25)}...] @ ${c} (${companyCounts[c] || 1} total)`);
        });
        console.log("   ----------------------------------------\n");
    ...
    // Helper to ensure UI is present
    const ensureControls = async (page) => {
        try {
            const hasControls = await page.evaluate(() => !!document.getElementById('jobright-controls'));
            if (!hasControls) {
                console.log("   🛠️  Re-injecting Controls Overlay...");
                await INJECT_CONTROLS_SCRIPT(page); // Re-run the injection function
            }
        } catch (e) { }
    };
    ...
    while (true) {
        ...
        for (let i = 0; i < jobs.length; i++) {
            ...
            const job = jobs[i];
            console.log(`   🔎 Checking Job ${i + 1}/${jobs.length}: ${job.url.substring(0, 30)}...`); // DEBUG LOG

            // Re-check if applied (in case of duplicates in file)
            /*
            if (appliedUrls.has(normalizeUrl(job.url))) {
                console.log(`   ⏭️  Skipping (Already Applied/In-Memory): ${job.url}`); // DEBUG LOG
                continue;
            }
            */
            ...
            // DYNAMIC TIMEOUT: User requested 120s for all jobs
            let currentJobTimeout = JOB_TIMEOUT_MS;
            ...
            try {
                ...
                await INJECT_CONTROLS_SCRIPT(page);
                await INJECT_RECORDING_SCRIPT(page);
                ...
                if (!isPassiveMode) {
                    if (// currentUrlLower.includes('ashbyhq') || // ALLOWED
                        // currentUrlLower.includes('myworkdayjobs') || // ALLOWED
                        // currentUrlLower.includes('greenhouse') || // ALLOWED
                        // currentUrlLower.includes('lever') || // ALLOWED
                ...
                console.log(`   ! Navigation Failed: ${e.message}`);
                if (e.message.includes("Target page, context or browser has been closed") || e.message.includes("Session closed")) {
                    console.error("   🚨 FATAL BROWSER ERROR: Context died. Exiting to prevent loop.");
                    process.exit(1);
                }
            ...
            try {
                ...
                while (Date.now() - startTime < JOB_TIMEOUT_MS && actionCount < maxActions) {
                    ...
                    if (!isPassiveMode) {
                        ...
                        // ENABLE WORKDAY HEURISTICS IN ACTIVE MODE (Auto-Replay)
                        if (!bestCandidate && !isPassiveMode) {
                                ...
                                try {
                                    ...
                                    for (const el of els) {
                                        if (await el.isVisible() && await el.isEnabled()) {
                                            bestCandidate = el;
                                            candidateSource = `Workday Heuristic: ${sel}`;
                                            console.log(`   🎯 Workday Heuristic Matched: ${sel}`); // Added Log
                                            break;
                                        }
                                    }
                                } catch (e) { }
                    ...
                    if (bestCandidate) {
                        ...
                        try {
                            ...
                            if (isPassiveMode) {
                                ...
                                // FIX: Wait to handle potential user signals (Skip/Delete) before continuing loop
                                bestCandidate = null;
                                const skipNow = await waitForActionOrSkip(page, 500); // Check signals during this block
                                if (skipNow) {
                ...
                console.log(`   ! Stack: ${e.stack}`); // Added Stack Trace
                jobError = e.message || "Unknown Error";
            ...
            try {
                if (jobStatus === "APPLIED") {
                    ...
                    // PHYSICAL DELETION (User Requirement: Remove from pool)
                    try {
                        if (fs.existsSync(JOBS_FILE)) {
                    ...
                    } catch (err) { console.error(`      ! Failed to delete from file:`, err.message); }
